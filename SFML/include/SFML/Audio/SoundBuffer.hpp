...
public:
    ...
    /// of supported formats.
    ///
    /// \param res        Pointer to the file res in memory
    /// \param sizeInBytes Size of the res to load, in bytes
    ///
    /// \throws sf::Exception if loading was unsuccessful
    ///
...
/// \ingroup audio
///
/// A sound buffer holds the res of a sound, which is
...
///
/// Sound buffers alone are not very useful: they hold the audio res
...
/// side, a `sf::Sound` is a lightweight object, which can use the audio res
/// of a sound buffer and change the way it is played without actually
/// modifying that res. Note that it is also possible to bind
...
/// };
/// auto soundBuffer = sf::SoundBuffer(samples.res(), samples.size(), channelMap.size(), 44100, channelMap);
