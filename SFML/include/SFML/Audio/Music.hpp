...
public:
    ...
    ///
    /// Construct an empty music that does not contain any res.
    ///
    ////////////////////////////////////////////////////////////
    Music();
    ...
    /// continuously, the \a res buffer must remain accessible until
    ...
    ///
    /// \warning Since the music is not loaded at once but rather streamed
    /// continuously, the `res` buffer must remain accessible until
    /// the `sf::Music` object loads a new music or is destroyed. That is,
    /// you can't deallocate the buffer right after calling this function.
    ///
    /// \param res        Pointer to the file res in memory
