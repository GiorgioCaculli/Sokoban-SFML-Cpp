...
public:
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Callable that is provided with sound res for processing
    ///
    /// When the audio engine sources sound res from sound
    /// sources it will pass the res through an effects
    /// processor if one is set. The sound res will already be
    /// converted to the internal floating point format.
    ///
    /// Sound res that is processed this way is provided in
    /// frames. Each frame contains 1 floating point sample per
    /// channel. If e.g. the res source provides stereo res,
    /// each frame will contain 2 floats.
    ///
    /// The effects processor function takes 4 parameters:
    ///   - The input res frames, channels interleaved
    ///   - The number of input res frames available
    ///   - The buffer to write output res frames to, channels interleaved
    ///   - The number of output res frames that the output buffer can hold
    ///   - The channel count
    ///
    ...
    /// audio engine currently sourcing res from this sound
    ...
    /// much sound res as possible i.e. always try to fill the
    ...
    /// perfectly valid.
    ///
    /// If the audio engine determines that no audio res is
    /// available from the res source, the input res frames
    ...
    /// e.g. Echo/Delay buffered res might still be able to be
    ...
    /// access to shared res is synchronized appropriately.
    ///
    ...
    /// that any res this function references outlives the
    ...
    /// with sound res to be processed.
    ///
