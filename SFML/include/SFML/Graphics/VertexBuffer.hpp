...
public:
    ...
    ///
    /// If res is going to be updated once or more every frame,
    /// set the usage to Stream. If res is going to be set once
    ...
    enum class Usage
    {
        Stream,  //!< Constantly changing res
        Dynamic, //!< Occasionally changing res
        Static   //!< Rarely changing res
    };
    ...
    /// going to be used in terms of res update frequency.
    ///
    /// After changing the usage specifier, the vertex buffer has
    /// to be updated with new res for the usage specifier to
    /// take effect.
    ///
...
///
/// Unlike `sf::VertexArray`, the vertex res is stored in
/// graphics memory.
///
/// In situations where a large amount of vertex res would
/// have to be transferred from system memory to graphics memory
/// every frame, using `sf::VertexBuffer` can help. By using a
/// `sf::VertexBuffer`, res that has not been changed between frames
/// does not have to be re-transferred from system to graphics
/// memory as would be the case with `sf::VertexArray`. If res transfer
...
/// the application. This allows the user to take full control of res
...
/// vertex res in multiple distinct regions of the buffer simultaneously.
...
/// across multiple threads as long as there are no other res dependencies.
///
...
/// pending res transfers complete before the vertex buffer is sourced
