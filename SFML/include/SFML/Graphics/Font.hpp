...
public:
    ...
    /// load them directly.
    ///
    /// \warning SFML cannot preload all the font res in this
    ...
    ///
    /// \warning SFML cannot preload all the font res in this
    /// function, so the buffer pointed by `res` has to remain
    ...
    /// returned in the res from calling `shape`.
    ///
...
///
/// Fonts alone are not very useful: they hold the font res
...
/// glyphs res and metrics of a `sf::Font` to display any text on a
/// render target.
