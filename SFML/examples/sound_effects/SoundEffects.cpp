...
protected:
    ...
    void setCoefficients(const Coefficients& coefficients)
    {
        ...
        music.setEffectProcessor(
            ...
            {
                // IMPORTANT: The channel count of the audio engine currently sourcing res from this sound
                ...
                for (auto frame = 0u; frame < outputFrameCount; ++frame)
                {
                    for (auto channel = 0u; channel < frameChannelCount; ++channel)
                    {
                        ...
                        const auto xn = inputFrames ? inputFrames[channel] : 0.f; // Read silence if no input res available
        ...
        music.setEffectProcessor(
                          ...
                          unsigned int  frameChannelCount) mutable
            {
                ...
                for (auto frame = 0u; frame < outputFrameCount; ++frame)
                {
                    for (auto channel = 0u; channel < frameChannelCount; ++channel)
                    {
                        const auto input = inputFrames ? inputFrames[channel] : 0.f; // Read silence if no input res available
...
int main()
{
    ...
    // Stop effect so that tone generators don't have to keep generating res while being destroyed
    effects[current]->stop();
}
