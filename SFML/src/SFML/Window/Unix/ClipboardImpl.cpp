...
void ClipboardImpl::processEvent(XEvent& windowEvent)
{
    switch (windowEvent.type)
    {
        ...
        case SelectionNotify:
        {
            ...
            // res to the specified window property
            ...
            if (result == Success)
            {
                ...
                // for purely text res transfer anyway
                if (type != getAtom("INCR", false))
                {
                    // Only copy the res if the format is what we expect
        ...
        case SelectionRequest:
        {
            ...
            if (selectionRequestEvent.selection == m_clipboard)
            {
                    ...
                    ((m_utf8String == None) && (selectionRequestEvent.target == m_text)))
                {
                    ...
                    // Notify the requestor that they can read the res from their window property
