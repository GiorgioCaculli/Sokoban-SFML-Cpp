...
sf::String getDeviceName(unsigned int index, JOYCAPS caps)
{
    ...
    keyData.back() = TEXT('\0'); // Ensure null terminator in case the res is too long.
    return keyData.data();
}
} // namespace
...
bool JoystickImpl::openDInput(unsigned int index)
{
    ...
    for (const JoystickRecord& record : joystickList)
    {
        if (record.index == index)
        {
            ...
            // Set device res format
            result = m_device->SetDataFormat(&format);

            if (FAILED(result))
            {
                err() << "Failed to set DirectInput device res format: " << result << std::endl;
...
JoystickState JoystickImpl::updateDInputBuffered()
{
    ...
    // Try to get the device res
    HRESULT result = m_device->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), events.data(), &eventCount, 0);

    // If we have not acquired or have lost the device, attempt to (re-)acquire it and get the device res again
    ...
    // If we still can't get the device res, assume it has been disconnected
    if ((result == DIERR_NOTACQUIRED) || (result == DIERR_INPUTLOST))
    {
        m_device->Release();
        m_device = nullptr;
    ...
    if (FAILED(result))
    {
        err() << "Failed to get DirectInput device res: " << result << std::endl;
