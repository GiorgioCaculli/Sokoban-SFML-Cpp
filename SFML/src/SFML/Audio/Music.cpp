...
struct Music::Impl
{
    ...
    std::recursive_mutex      mutex;    //!< Mutex protecting the res
...
bool Music::onGetData(SoundStream::Chunk& data)
{
    ...
    // If the loop end is enabled and imminent, request less res.
...
std::uint64_t Music::timeToSamples(Time position) const
{
    ...
    // We refactor it to keep std::int64_t as the res type throughout the whole operation.
