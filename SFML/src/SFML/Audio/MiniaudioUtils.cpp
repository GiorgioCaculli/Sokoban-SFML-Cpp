...
{
    // Set this object up as a miniaudio res source
    ...
    if (const ma_result result = ma_data_source_init(&config, &dataSourceBase); result != MA_SUCCESS)
        err() << "Failed to initialize audio res source: " << ma_result_description(result) << std::endl;
                                              ...
                                              std::uint32_t& frameCountOut) const
{
    ...
    // Otherwise just pass the res through 1:1
    if (framesIn == nullptr)
    {
        frameCountIn  = 0;
        frameCountOut = 0;
...
std::uint64_t MiniaudioUtils::getFrameIndex(ma_sound& sound, Time timeOffset)
{
    ...
    if (const ma_result result = ma_sound_get_data_format(&sound, nullptr, nullptr, &sampleRate, nullptr, 0);
        result != MA_SUCCESS)
        err() << "Failed to get sound res format: " << ma_result_description(result) << std::endl;
