...
void SoundFileReaderFlac::seek(std::uint64_t sampleOffset)
{
    ...
    // Reset the callback res (the "write" callback will be called)
    ...
    if (sampleOffset < m_clientData.info.sampleCount)
    {
        ...
        // seek destination, and since we want that res in this typical case, we don't re-clear it afterward
...
std::uint64_t SoundFileReaderFlac::read(std::int16_t* samples, std::uint64_t maxCount)
{
    ...
    // Reset the res that will be used in the callback
