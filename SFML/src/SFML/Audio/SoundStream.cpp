...
struct SoundStream::Impl : priv::MiniaudioUtils::SoundBase
{
    ...
    void initialize()
    {
        ...
        // Because we are providing a custom res source, we have to provide the channel map ourselves
        if (!channelMap.empty())
        {
    ...
    static ma_result read(ma_data_source* dataSource, void* framesOut, std::uint64_t frameCount, std::uint64_t* framesRead)
    {
        ...
        // Try to fill our buffer with new samples if the source is still willing to stream res
