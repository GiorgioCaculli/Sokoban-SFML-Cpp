...
Socket::Status UdpSocket::send(const void* data, std::size_t size, IpAddress remoteAddress, unsigned short remotePort)
{
    ...
    // Make sure that all the res will fit in one datagram
    if (size > MaxDatagramSize)
    {
        err() << "Cannot send res over the network "
...
#pragma GCC diagnostic ignored "-Wuseless-cast"
    // Send the res (unlike TCP, all the res is always sent in one call)
                                  ...
                                  unsigned short&           remotePort)
{
    ...
    if (!data)
    {
        err() << "Cannot receive res from the network (the destination buffer is invalid)" << std::endl;
        return Status::Error;
    }
...
Socket::Status UdpSocket::send(Packet& packet, IpAddress remoteAddress, unsigned short remotePort)
{
    ...
    // Sending one datagram is almost safe: it may be lost but if it's received, then its res
    ...
    // Get the res to send from the packet
    std::size_t size = 0;
...
Socket::Status UdpSocket::receive(Packet& packet, std::optional<IpAddress>& remoteAddress, unsigned short& remotePort)
{
    ...
    // If we received valid res, we can copy it to the user packet
    packet.clear();
