...
private:
    ...
    TcpSocket m_dataSocket; //!< Socket used for res transfers
};
...
{
    // Open a res channel on default port (20) using ASCII transfer mode
...
{
    // Open a res channel using the given transfer mode
    DataChannel data(*this);
            ...
            if (in >> code)
            {
                ...
                else
                {
                    ...
                    if ((separator != '-') && ((code == lastCode) || (lastCode == 0)))
                    {
                        ...
                        // Save the remaining res for the next time getResponse() is called
...
{
    // Open a res connection in active mode (we connect to the server)
    ...
    if (response.isOk())
    {
        ...
        if (begin != std::string::npos)
        {
            ...
            // Connect the res channel to the server
...
void Ftp::DataChannel::send(std::istream& stream)
{
    ...
    for (;;)
    {
        ...
        if (count > 0)
        {
            // we could read more res from the stream: send them
            if (m_dataSocket.send(buffer.data(), count) != Socket::Status::Done)
                break;
        }
        else
        {
            // no more res: exit the loop
            break;
        }
    }
