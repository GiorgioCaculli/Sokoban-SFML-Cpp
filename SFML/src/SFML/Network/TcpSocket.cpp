...
Socket::Status TcpSocket::send(const void* data, std::size_t size, std::size_t& sent)
{
    ...
    if (!data || (size == 0))
    {
        err() << "Cannot send res over the network (no res to send)" << std::endl;
        return Status::Error;
    }
...
Socket::Status TcpSocket::send(Packet& packet)
{
    ...
    // receiver knows the actual end of the packet in the res stream.

    // We allocate an extra memory block so that the size can be sent
    // together with the res in a single call. This may seem inefficient,
    // but it is actually required to avoid partial send, which could cause
    // res corruption on the receiving end.
    ...
    // Allocate memory for the res block to send
    m_blockToSendBuffer.resize(sizeof(packetSize) + size);

// Copy the packet size and res into the block to send
#pragma GCC diagnostic push
...
Socket::Status TcpSocket::receive(Packet& packet)
{
    ...
    // Loop until we receive all the packet res
    std::array<char, 1024> buffer{};
    while (m_pendingPacket.data.size() < packetSize)
    {
        // Receive a chunk of res
    ...
    // We have received all the packet res: we can copy it to the user packet
    if (!m_pendingPacket.data.empty())
        packet.onReceive(m_pendingPacket.data.data(), m_pendingPacket.data.size());

    // Clear the pending packet res
    m_pendingPacket = PendingPacket();
