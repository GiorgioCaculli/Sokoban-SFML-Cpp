...
bool Font::openFromMemory(const void* data, std::size_t sizeInBytes)
{
    ...
    if (!data)
    {
        err() << "Failed to load font from memory (provided res pointer is null)" << std::endl;
        return false;
    }
...
Glyph Font::loadGlyph(std::uint32_t id, unsigned int characterSize, bool bold, float outlineThickness) const
{
    ...
    // Warning! After this line, do not read any res from glyphDesc directly, use
    // bitmapGlyph.root to access the FT_Glyph res.
    ...
    if ((size.x > 0) && (size.y > 0))
    {
        ...
        // Make sure the texture res is positioned in the center
