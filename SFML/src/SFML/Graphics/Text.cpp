...
struct Text::ShaperImpl
{
    ...
    // The glyph res that is output after shaping
    struct GlyphData
    {
        ...
        std::uint32_t                     style)
    {
        ...
        // Retrieve position res glyph by glyph
        ...
        // HarfBuzz returns position res in scaled units
...
void Text::ensureGeometryUpdate() const
{
    ...
    const auto outputLine = [&]
    {
        ...
        for (const auto& shapeGlyph : shapeOutput)
        {
            ...
            {
                // We only add res to the vertical underline list for base glyphs
    ...
    // In order to be able to align text we have to record all line res until we can compute the text metrics
    // We then use the record res to shift the necessary lines to the right/left as necessary
    struct LineRecord
    {
    ...
    // Use line record res to post-process lines e.g. re-alignment etc.
    if (!lines.empty())
    {
        ...
        for (auto& line : lines)
        {
            ...
            {
                // Skip modifying the res if there is nothing to shift
                continue;
            }
