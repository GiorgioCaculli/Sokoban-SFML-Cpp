/* TODO: Apply decorator design pattern to this class */
...
#include <gui/entities/Entity_Platform.hpp>
#include <gui/entities/Entity_Wall.hpp>
 ...
 */
World::World(sf::RenderTarget &target, const gzc::sokoban::core::Board &board,
             Font_Holder &fonts, Sound_Player &sounds)
    : _target(target), _world_view(target.getDefaultView()), _fonts(fonts),
      _sounds(sounds), _player_is_moving_up(false),
      _player_is_moving_down(false), _player_is_moving_left(false),
      _player_is_moving_right(false),
      _world_bounds(
          sf::Vector2f(0.f, 0.f),
          sf::Vector2f(_world_view.getSize().x, _world_view.getSize().y)),
      _board(""), _player_sprite(nullptr), _board_player(nullptr),
      _player_entity(nullptr), _box_texture_sheet(nullptr),
      _platform_texture_sheet(nullptr), _wall_texture_sheet(nullptr),
      _player_texture_sheet(nullptr), _background_texture(nullptr),
  ...
  box_move_buffer.loadFromFile( "res/sounds/wood_creak_01.ogg" );
...
void World::update(const sf::Time dt) {
  ...
  if (_player_is_moving_up) {
    ...
    player_assets_coordinates = sf::IntRect(
        sf::Vector2i(player_x_coordinates, player_y_coordinates),
    ...
    for (long unsigned int i = 0; i < _box_entities.size(); i++) {
      _box_sprites.at(i)->setPosition(
          sf::Vector2f(_box_actors.at(i)->get_x() * OFFSET,
  ...
  if (_player_is_moving_left) {
    const auto player_asset_rect = _player_entity->get_player_face_map()
                                       .find(entity::Entity_Player::Face::WEST)
                                       ->second;
    player_x_coordinates = player_asset_rect.at(0);
    player_y_coordinates = player_asset_rect.at(1);
    player_width_coordinates = player_asset_rect.at(2);
  ...
  if (_player_is_moving_right) {
    const auto player_asset_rect = _player_entity->get_player_face_map()
                                       .find(entity::Entity_Player::Face::EAST)
                                       ->second;
    player_x_coordinates = player_asset_rect.at(0);
    player_y_coordinates = player_asset_rect.at(1);
    player_width_coordinates = player_asset_rect.at(2);
...
void World::load_textures() {
  ...
  switch (std::uniform_int_distribution background_distribution(
...
void World::build_scene() {
  ...
  std::uniform_int_distribution box_distribution(min_within_enum,
                                                 max_within_enum);
  auto random_box_color =
      static_cast<entity::Entity_Box::Color>(box_distribution(mt));
  ...
  std::uniform_int_distribution platform_distribution(min_within_enum,
                                                      max_within_enum);
  auto random_platform_color =
      static_cast<entity::Entity_Platform::Color>(platform_distribution(mt));
  ...
  std::uniform_int_distribution wall_distribution(min_within_enum,
                                                  max_within_enum);
  auto random_wall_color =
      static_cast<entity::Entity_Wall::Color>(wall_distribution(mt));
 ...
 */
bool World::is_board_completed() const { return _board.is_completed(); }
 ...
 */
int World::get_reset_counter() const { return _reset_counter; }
